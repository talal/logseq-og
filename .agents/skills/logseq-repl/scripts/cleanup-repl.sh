...
usage() {
  cat <<'EOF'
Stop processes started by start-repl.sh.
...
if [[ ! -d "$LOG_DIR" && ! -d "$LEGACY_DESKTOP_LOG_DIR" ]]; then
...
stop_by_pid_file "$LOG_DIR/shared-shadow-watch.pid" "desktop watcher"
stop_by_pid_file "$LEGACY_DESKTOP_LOG_DIR/desktop-electron.pid" "legacy desktop-electron"
stop_by_pid_file "$LEGACY_DESKTOP_LOG_DIR/shadow-watch.pid" "legacy desktop watcher"

rm -f "$LOG_DIR"/*.pid "$LEGACY_DESKTOP_LOG_DIR"/*.pid 2>/dev/null || true

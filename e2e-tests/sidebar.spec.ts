...
test('favorite item and recent item test', async ({ page }) => {
  ...
  expect(await page.innerText(':nth-match(.favorite-item a, 1)')).toBe(
    fav_page_name
  )
  ...
  expect(await page.innerText(':nth-match(.recent-item a, 1)')).toBe(
    fav_page_name
  )
  expect(await page.innerText(':nth-match(.recent-item a, 2)')).toBe(
    another_page_name
  )

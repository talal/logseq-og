...
if (!IsMac) {
  ...
  test('window should be maximized and icon should change on maximize-toggle click', async ({
    page,
  }) => {
    await page.click('.window-controls .maximize-toggle.maximize')

    await expect(
      page.locator('.window-controls .maximize-toggle.restore')
    ).toHaveCount(1)
  })

  test('window should be restored and icon should change on maximize-toggle click', async ({
    page,
  }) => {
  ...
  test('window controls should be hidden on fullscreen mode', async ({
    page,
  }) => {
  ...
  test('window controls should be visible when we exit fullscreen mode', async ({
    page,
  }) => {

...
import {
  LSAuthenticator,
  LSAuthenticatorChangePassword,
} from './LSAuthenticator'
...
dict.zh['Incorrect username or password.'] =
  '用户名或者密码不正确。如果您的邮箱未验证，请尝试使用用户名(非邮箱)登录，以保证再次邮箱验证流程。'
...
function setupAuthConfigure(config) {
  ...
  Amplify.configure({
    ...
    aws_cognito_social_providers: oauthProviders || ['GOOGLE'],

...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    <DialogOverlay>
      ...
      >
        {children}
        <DialogPrimitive.Close className="ui__dialog-close absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground">
          <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description
    ref={ref}
    className={cn(
      'ui__dialog-description text-sm text-muted-foreground',
      className
    )}
    {...props}
  />
